...
public interface IHealthMonitorType : IServiceType
{
    ...
    /// <c>HealthMonitorTypes</c>'s own <c>AddScoped</c> factory via <see cref="Register"/>, once per
    ...
    /// same scope whose <c>IServiceProvider</c> the closure resolves against. The prior approach
    /// registered from each option's <c>Initialize</c>, which only ever runs once against the root
    ...
    /// e.g. this option is only being asked to register its configuration), the domain configuration
    /// provider (or null if none is available), and the logger the caller already built (with its
    /// own <c>NullLogger</c> fallback already applied) — not re-resolved here, so every option logs
    /// through the SAME logger instance the domain provider's own construction used.
    /// </param>
    void Registration(Func<IServiceProvider, IHealthMonitorProvider?, IHealthMonitorConfigurationProvider?, ILogger<IHealthMonitorType>, IGenericResult> method);
    ...
    IGenericResult Register(IServiceProvider serviceProvider, IHealthMonitorProvider? domainProvider, IHealthMonitorConfigurationProvider? domainConfigurationProvider, ILogger<IHealthMonitorType> logger);
}
