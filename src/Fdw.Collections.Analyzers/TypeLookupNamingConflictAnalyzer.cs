...
/// will generate a method that conflicts with a member in the TypeCollection's inheritance chain.
/// </summary>
...
public class TypeLookupNamingConflictAnalyzer : DiagnosticAnalyzer
{
    ...
    /// </summary>
    public const string DiagnosticId = "TYPECOLL001";
    ...
    private static readonly LocalizableString Description =
        "When a TypeLookup attribute generates a method with the same name as a member in the TypeCollection's inheritance chain, it causes CS0108 warnings. Use a custom lookup method name to avoid the conflict.";
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterSymbolAction(AnalyzeTypeCollection, SymbolKind.NamedType);
    }

    private static void AnalyzeTypeCollection(SymbolAnalysisContext context)
    {
        var typeSymbol = (INamedTypeSymbol)context.Symbol;

        // Check if this is a TypeCollection (has [TypeCollection] attribute)
        var typeCollectionAttr = typeSymbol.GetAttributes()
            .FirstOrDefault(ad => string.Equals(ad.AttributeClass?.Name, "TypeCollectionAttribute", System.StringComparison.Ordinal));

        if (typeCollectionAttr == null)
            return;

        // Get the base type from the TypeCollection attribute (first constructor argument)
        if (typeCollectionAttr.ConstructorArguments.Length == 0 ||
            typeCollectionAttr.ConstructorArguments[0].Value is not INamedTypeSymbol baseType)
            return;

        // Collect all members from the TypeCollection's inheritance chain
