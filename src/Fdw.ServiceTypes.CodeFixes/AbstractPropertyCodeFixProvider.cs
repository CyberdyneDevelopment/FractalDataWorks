...
namespace Fdw.ServiceTypes.CodeFixes;
...
#pragma warning disable FDW006, FDW007
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Find all derived types BEFORE we start editing
        ...
        // Get the current document root to find derived classes in the same file
        var root = await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(false);
        if (root == null) return solution;

        // Find derived classes in the same document
        var derivedClassesInSameDocument = root.DescendantNodes()
            .OfType<ClassDeclarationSyntax>()
            .Where(c =>
            {
                var symbol = semanticModel.GetDeclaredSymbol(c, cancellationToken);
                return symbol != null && derivedTypes.Any(dt => SymbolEqualityComparer.Default.Equals(dt, symbol));
            })
            .ToList();
        ...
        {
            // First pass: Update constructors that don't have this() initializers
            var constructorsToUpdate = constructors.Where(c => c.Initializer?.Kind() != SyntaxKind.ThisConstructorInitializer).ToList();
            foreach (var constructor in constructorsToUpdate)
            {
            ...
            // Second pass: Update this() initializers to pass default value for new parameter
            var constructorsWithThis = constructors.Where(c => c.Initializer?.Kind() == SyntaxKind.ThisConstructorInitializer).ToList();
            foreach (var constructor in constructorsWithThis)
            {
                if (constructor.Initializer == null)
                    continue;

                // Create default value for the parameter
                ExpressionSyntax defaultValue;
                var typeString = propertyType.ToString();
                if (string.Equals(typeString, "string", StringComparison.Ordinal))
                {
                    defaultValue = SyntaxFactory.LiteralExpression(SyntaxKind.StringLiteralExpression, SyntaxFactory.Literal(string.Empty));
                }
                else if (string.Equals(typeString, "bool", StringComparison.Ordinal))
                {
                    defaultValue = SyntaxFactory.LiteralExpression(SyntaxKind.FalseLiteralExpression);
                }
                else if (string.Equals(typeString, "int", StringComparison.Ordinal))
                {
                    defaultValue = SyntaxFactory.LiteralExpression(SyntaxKind.NumericLiteralExpression, SyntaxFactory.Literal(0));
                }
                else
                {
                    defaultValue = SyntaxFactory.DefaultExpression(propertyType);
                }

                // Add argument to this() initializer
                var newInitializer = constructor.Initializer.WithArgumentList(
                    constructor.Initializer.ArgumentList.AddArguments(
                        SyntaxFactory.Argument(defaultValue)));

                var newConstructor = constructor.WithInitializer(newInitializer);
                editor.ReplaceNode(constructor, newConstructor);
            }
        }

        // Update derived classes in the same document
        foreach (var derivedClass in derivedClassesInSameDocument)
        {
            UpdateDerivedClassInSameDocument(editor, derivedClass, propertySymbol, semanticModel, cancellationToken);
        }
        ...
        foreach (var derivedType in derivedTypes)
        {
            foreach (var location in derivedType.Locations)
            {
                ...
                {
                    // Find the document by file path since the syntax tree may have changed
                    var filePath = location.SourceTree.FilePath;
                    var derivedDocument = solution.Projects
...
#pragma warning restore FDW006, FDW007
    ...
    // Roslyn document transformation with Syntax API — sequential syntax node updates
#pragma warning disable FDW006, FDW007
    private static void UpdateDerivedClassInSameDocument(
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var member in derivedClass.Members.OfType<PropertyDeclarationSyntax>())
        {
        ...
        // Remove the overridden property if found
...
#pragma warning restore FDW019
        ...
        var constructors = derivedClass.Members.OfType<ConstructorDeclarationSyntax>().ToList();

        if (constructors.Count == 0)
        {
            // Create a new constructor that calls base with the property value
            var constructor = SyntaxFactory.ConstructorDeclaration(derivedClass.Identifier)
            ...
            editor.InsertMembers(derivedClass, 0, new[] { constructor });
        }
        else
        {
...
#pragma warning restore FDW019
        ...
        if (constructors.Count == 0)
        {
            ...
            editor.InsertMembers(classDeclaration, 0, new[] { constructor });
        }
        else
        {
            foreach (var constructor in constructors)
