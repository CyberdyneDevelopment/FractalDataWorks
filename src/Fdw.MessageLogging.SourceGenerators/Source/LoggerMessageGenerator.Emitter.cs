    ...
    public partial class LoggerMessageGenerator
    {
        internal sealed class Emitter(Compilation compilation)
        {
            ...
            private readonly bool _hasStringCreate =
                compilation.GetSpecialType(SpecialType.System_String).GetMembers("Create").OfType<IMethodSymbol>()
                    .Any(m => m.IsStatic &&
                              ...
                              string.Equals(m.Parameters[0].Type.Name, "IFormatProvider", StringComparison.Ordinal) &&
            ...
            public string Emit(IReadOnlyList<LoggerClass> logClasses, CancellationToken cancellationToken)
            {
                ...
                _builder.AppendLine("#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member");
...
#pragma warning disable MA0051 // Source generator emits complete log struct — splitting scatters the template
...
#pragma warning disable MA0051 // Source generator emits complete log method with all overloads — splitting scatters the template
            [ConventionOverride(MaxCyclomaticComplexity = 17, MaxMethodLines = 142)]  // Code generation template for log methods — sequential string building
...
");
                ...
                _builder.Append($@"
        {nestedIndentation}/// <remarks>Auto-generated by <c>LoggerMessageGenerator</c>. To override, define a static method
        {nestedIndentation}/// with this exact signature in the partial class — the generator will detect it and skip generation.</remarks>
        {nestedIndentation}[{s_generatedCodeAttribute}]
        {nestedIndentation}{lm.Modifiers} global::Fdw.Messages.IGenericMessage {lm.Name}({extension}");
        ...
        {nestedIndentation}{{");
            ...
            {nestedIndentation}}}");
                ...
                // Generate IGenericMessage return statement
                string severityMapping = GetMessageSeverity(lm);
                _builder.Append($@"

            {nestedIndentation}// Create and return IGenericMessage
            {nestedIndentation}var __messageText = ");

                // Use the same logic as the struct ToString
                if (UseLoggerMessageDefine(lm))
                {
                    // For LoggerMessage.Define, we need to format the message manually
                    string formatMethodBegin =
                        !lm.Message.Contains('{') ? "" :
                        _hasStringCreate ? "string.Create(global::System.Globalization.CultureInfo.InvariantCulture, " :
                        "global::System.FormattableString.Invariant(";
                    ...
                    _builder.Append($@"{formatMethodBegin}$""{ConvertEndOfLineAndQuotationCharactersToEscapeForm(lm.Message)}""{formatMethodEnd};");
                }
                else
                {
                    // For struct-based logging, use the struct's ToString
                    _builder.Append($@"");
                    GenHolder(lm);
                    _builder.Append($@".ToString();");
                }

                string messageCode = $"\"{lm.TypeCode}-{lm.EventId}\"";
                _builder.Append($@"
            {nestedIndentation}return global::Fdw.Messages.GenericMessage.Create(
                {nestedIndentation}    {severityMapping},
                {nestedIndentation}    __messageText,
                {nestedIndentation}    {messageCode},
        ...
        {nestedIndentation}}}");
                ...
                static string GetMessageSeverity(LoggerMethod lm)
                {
                    // Check for custom MessageSeverity from attribute
                    if (!lm.AutoMapSeverity && lm.Severity != null)
                    {
                        return lm.Severity.Value switch
                        {
                            0 => "global::Fdw.Messages.MessageSeverity.Debug",
                            1 => "global::Fdw.Messages.MessageSeverity.Information",
                            2 => "global::Fdw.Messages.MessageSeverity.Warning",
                            3 => "global::Fdw.Messages.MessageSeverity.Error",
                            4 => "global::Fdw.Messages.MessageSeverity.Critical",
                            _ => "global::Fdw.Messages.MessageSeverity.Information",
                        };
                    }

                    // Fall back to LogLevel-based mapping
                    if (lm.Level == null)
                    {
                        return "global::Fdw.Messages.MessageSeverity.Information";
                    }

                    return lm.Level switch
                    {
                        0 => "global::Fdw.Messages.MessageSeverity.Debug",     // Trace -> Debug
                        1 => "global::Fdw.Messages.MessageSeverity.Debug",     // Debug -> Debug
                        2 => "global::Fdw.Messages.MessageSeverity.Information", // Information
                        3 => "global::Fdw.Messages.MessageSeverity.Warning",   // Warning
                        4 => "global::Fdw.Messages.MessageSeverity.Error",     // Error
                        5 => "global::Fdw.Messages.MessageSeverity.Critical",  // Critical
                        6 => "global::Fdw.Messages.MessageSeverity.Information", // None -> Information
                        _ => "global::Fdw.Messages.MessageSeverity.Information",
                    };
                }
            }
