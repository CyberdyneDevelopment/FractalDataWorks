...
namespace Fdw.MessageLogging.Generators
{
    public partial class LoggerMessageGenerator
    {
        ...
        {
            internal const string LoggerMessageAttribute = "Fdw.MessageLogging.MessageLoggingAttribute";
            ...
            /// </summary>
#pragma warning disable MA0051 // Parser validates attributes, parameters, return types, and diagnostics — must stay as one cohesive pass
            [ConventionOverride(MaxCyclomaticComplexity = 147, MaxMethodLines = 357)]  // Parser entry point — validates attributes, parameters, return types across all log classes
            public IReadOnlyList<LoggerClass> GetLogClasses(IEnumerable<ClassDeclarationSyntax> classes)
            {
                ...
                INamedTypeSymbol? genericMessageSymbol = _compilation.GetBestTypeByMetadataName("Fdw.Messages.IGenericMessage");
                if (genericMessageSymbol == null)
                {
                    Diag(DiagnosticDescriptors.MissingRequiredType, null, "Fdw.Messages.IGenericMessage");
                    return Array.Empty<LoggerClass>();
                }
                        ...
                        foreach (MemberDeclarationSyntax member in classDec.Members)
                        {
                            ...
                            foreach (AttributeListSyntax mal in method.AttributeLists)
                            {
                                foreach (AttributeSyntax ma in mal.Attributes)
                                {
                                                ...
                                                else
                                                {
                                                    ...
                                                    switch (namedArgument.Key)
                                                    {
                                                        ...
                                                        case "Message":
                                                            ...
                                                            break;
                                                        case "Severity":
                                                            severity = value.IsNull ? null : (int?)GetItem(value);
                                                            break;
                                                        case "AutoMapSeverity":
                                                            autoMapSeverity = (bool)GetItem(value);
                                                            break;
                                                        case "TypeCode":
                                                            if (value.Kind == TypedConstantKind.Array && !value.IsNull)
                                                            {
                                                                var chars = new char[value.Values.Length];
                                                                for (int ci = 0; ci < value.Values.Length; ci++)
                                                                {
                                                                    chars[ci] = (char)value.Values[ci].Value!;
                                                                }
                                                                typeCode = new string(chars);
                                                            }
                                                            break;
                                                    }
                                                }
                                    ...
                                    // Resolve TypeCode: per-method override -> class-level
                                    // [MessageLoggingTypeCode("...")] default -> "FDW".
                                    string resolvedTypeCode = typeCode ?? ResolveClassTypeCode(logMethodSymbol!) ?? "FDW";
                                    if (resolvedTypeCode.Length < 2 || resolvedTypeCode.Length > 16)
                                    {
                                        Diag(DiagnosticDescriptors.InvalidTypeCode, method.Identifier.GetLocation(), resolvedTypeCode);
                                    }
                                    else
                                    {
                                        foreach (char c in resolvedTypeCode)
                                        {
                                            if (!char.IsLetterOrDigit(c) || !char.IsUpper(c) && !char.IsDigit(c))
                                            {
                                                Diag(DiagnosticDescriptors.InvalidTypeCode, method.Identifier.GetLocation(), resolvedTypeCode);
                                                break;
                                            }
                                    ...
                                    var lm = new LoggerMethod
                                    {
                                        ...
                                        Severity = severity,
                                        AutoMapSeverity = autoMapSeverity,
...
#pragma warning disable FDW019 // External Roslyn RefKind enum — cannot convert to TypeCollection
...
#pragma warning restore FDW019
                                    ...
                                    if (lc == null)
                                    {
...
#endif
                                        {
                                            ...
                                            while (true)
                                            {
                                                NamespaceDeclarationSyntax? parentNs = namespaceParent.Parent as NamespaceDeclarationSyntax;
                                                if (parentNs == null)
                                                {
                                                    break;
                                                }

                                                namespaceParent = parentNs;
                        ...
                        if (lc != null)
                        {
                            ...
                            var methods = new Dictionary<string, int>(lc.Methods.Count, StringComparer.Ordinal);
            ...
            private static string GenerateClassName(TypeDeclarationSyntax typeDeclaration)
            {
                    ...
                    typeDeclaration.TypeParameterList.Parameters.Count != 0)
                {
                    ...
                    for (int i = 0; i < typeDeclaration.TypeParameterList!.Parameters.Count; i++)
                    {
                        ...
                        {
                            typeDeclaration = typeDeclaration.ReplaceNode(parameter, parameter.WithAttributeLists(default));
                        }
                    }
                }
...
#pragma warning disable MA0051 // Walks class hierarchy to find logger fields — must inspect all members cohesively
            [ConventionOverride(MaxCyclomaticComplexity = 19)]  // Roslyn symbol inspection walking inheritance chain — nested member traversal
            ...
            /// <summary>
            /// Checks if the return type is IGenericMessage or implements IGenericMessage.
            /// </summary>
            private static bool IsCompatibleReturnType(ITypeSymbol? returnType, INamedTypeSymbol? genericMessageSymbol)
            {
                if (returnType == null || genericMessageSymbol == null)
                {
                    return false;
                }

                // Check if it's exactly IGenericMessage
                if (SymbolEqualityComparer.Default.Equals(returnType, genericMessageSymbol))
                {
                    return true;
                }

                // Check if it implements IGenericMessage
                if (returnType is INamedTypeSymbol namedType)
                {
                    foreach (var @interface in namedType.AllInterfaces)
                    {
                        if (SymbolEqualityComparer.Default.Equals(@interface, genericMessageSymbol))
                        {
                            return true;
                        }
                    }
        ...
        internal sealed class LoggerMethod
        {
            ...
            public int? Severity;
            public bool AutoMapSeverity = true;
            public string TypeCode = "FDW";
        }
        ...
        /// <summary>
        /// Reads the class-level [MessageLoggingTypeCode("...")] default for a logging method's
        /// containing type, or null when none is declared.
        /// </summary>
        private static string? ResolveClassTypeCode(IMethodSymbol logMethodSymbol)
        {
            foreach (AttributeData ca in logMethodSymbol.ContainingType.GetAttributes())
            {
                if (string.Equals(ca.AttributeClass?.Name, "MessageLoggingTypeCodeAttribute", StringComparison.Ordinal)
                    && ca.ConstructorArguments.Length == 1
                    && ca.ConstructorArguments[0].Value is string cv
                    && cv.Length > 0)
                {
                    return cv;
                }
