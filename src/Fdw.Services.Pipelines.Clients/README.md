# Fdw.Services.Pipelines.Clients

The API client for pipelines and pipeline jobs.
...
| Type | Kind | Purpose |
|---|---|---|
| `PipelineHttpClient` | class | HTTP client implementation for the pipeline configuration API. |
| `PipelineJobHttpClient` | class | HTTP client implementation for the pipeline job service. |

## Installation

```bash
dotnet add package Fdw.Services.Pipelines.Clients --prerelease
```

## Dependencies

`Fdw.Services.Pipelines.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Http.Authentication`
