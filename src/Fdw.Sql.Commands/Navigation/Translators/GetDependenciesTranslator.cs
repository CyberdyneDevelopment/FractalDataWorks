...
public sealed class GetDependenciesTranslator : SqlCommandTranslatorBase<GetDependenciesCommand, QueryResult<string>>
{
    ...
    {
        var logger = NullLogger<GetDependenciesTranslator>.Instance;
        GetDependenciesTranslatorLog.Translating(logger, nameof(GetDependenciesCommand));
        return Task.FromResult<IGenericResult<QueryResult<string>>>(
            GenericResult<QueryResult<string>>.Failure(GetDependenciesTranslatorLog.NotYetImplemented(logger, nameof(GetDependenciesCommand))));
    }
}
