...
/// <summary>
/// Base class for the kinds of resource that can be attached to a dataverse.
/// </summary>
/// <remarks>
...
///
/// Nothing persists the derived id. <c>dataverse.DataverseResource.ResourceType</c> stores the
/// kind's name, so the name is the part of this contract that must not change once rows exist.
/// </remarks>
public abstract class DataverseResourceKindBase
    : TypeOptionBase<DataverseResourceKindBase>, IDataverseResourceKind
{
    /// <summary>
    /// Initializes a new instance of the <see cref="DataverseResourceKindBase"/> class.
    /// </summary>
    /// <param name="name">The kind name. This is the value persisted in the database.</param>
    /// <param name="canBeOwned">Whether a dataverse may own this kind, not merely use it.</param>
    protected DataverseResourceKindBase(string name, bool canBeOwned)
        : base(name)
    {
