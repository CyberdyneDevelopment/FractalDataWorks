...
public abstract class ListDataSetAnnotationsEndpoint : Endpoint<DataSetAnnotationRequest, List<DataSetAnnotationPayload>>
{
    private readonly QualityConfigurationProvider _provider;

    /// <summary>Initializes a new instance of the <see cref="ListDataSetAnnotationsEndpoint"/> class.</summary>
    /// <param name="provider">The configuration provider for quality and catalog data.</param>
    protected ListDataSetAnnotationsEndpoint(QualityConfigurationProvider provider)
    {
        _provider = provider;
    }
