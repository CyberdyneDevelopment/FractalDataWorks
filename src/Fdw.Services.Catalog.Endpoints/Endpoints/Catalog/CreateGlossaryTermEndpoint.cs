...
public abstract class CreateGlossaryTermEndpoint : Endpoint<GlossaryTermResponse, GlossaryTermResponse>
{
    private readonly QualityConfigurationProvider _provider;

    /// <summary>Initializes a new instance of the <see cref="CreateGlossaryTermEndpoint"/> class.</summary>
    /// <param name="provider">The configuration provider for quality and catalog data.</param>
    protected CreateGlossaryTermEndpoint(QualityConfigurationProvider provider)
    {
        _provider = provider;
    }
    ...
    public override async Task HandleAsync(GlossaryTermResponse req, CancellationToken ct)
    {
        ...
        await Send.CreatedAtAsync<GetGlossaryTermEndpoint>(new { Id = req.Id }, req, cancellation: ct).ConfigureAwait(false);
    }
}
