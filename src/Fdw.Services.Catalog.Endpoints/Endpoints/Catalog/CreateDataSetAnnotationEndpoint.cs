...
public abstract class CreateDataSetAnnotationEndpoint : Endpoint<DataSetAnnotationPayload, DataSetAnnotationPayload>
{
    private readonly QualityConfigurationProvider _provider;

    /// <summary>Initializes a new instance of the <see cref="CreateDataSetAnnotationEndpoint"/> class.</summary>
    /// <param name="provider">The configuration provider for quality and catalog data.</param>
    protected CreateDataSetAnnotationEndpoint(QualityConfigurationProvider provider)
    {
        _provider = provider;
    }
    ...
    public override async Task HandleAsync(DataSetAnnotationPayload req, CancellationToken ct)
    {
        ...
        await Send.CreatedAtAsync<GetDataSetAnnotationEndpoint>(
