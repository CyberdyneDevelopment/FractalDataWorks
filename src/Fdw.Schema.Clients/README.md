...
The API client for schema discovery operations.
...
| Type | Kind | Purpose |
|---|---|---|
| `SchemaApiClient` | class | API client for schema discovery and data preview endpoints. Implements with HTTP-backed operations… |
| `TableApiClient` | class | API client for physical database table management endpoints. |

## Installation

```bash
dotnet add package Fdw.Schema.Clients --prerelease
```

## Dependencies

`Fdw.Schema.Clients.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Endpoints` · `Fdw.Web.Http.Authentication`
