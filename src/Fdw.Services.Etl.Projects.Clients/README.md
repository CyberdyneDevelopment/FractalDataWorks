...
The API client for ETL projects and orchestration nodes.
...
| Type | Kind | Purpose |
|---|---|---|
| `NodeApiClient` | class | HTTP API client for generic orchestration node endpoints. Un-sealed with virtual methods for Moq… |
| `ProjectApiClient` | class | HTTP API client for ETL project orchestration endpoints. Un-sealed with virtual methods for Moq… |

## Installation

```bash
dotnet add package Fdw.Services.Etl.Projects.Clients --prerelease
```

## Dependencies

`Fdw.Services.Etl.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Http.Authentication`
