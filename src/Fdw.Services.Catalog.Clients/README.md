# Fdw.Services.Catalog.Clients

The API client for catalog operations.
...
| Type | Kind | Purpose |
|---|---|---|
| `CatalogApiClient` | class | API client for data catalog and search endpoints. |

## Installation

```bash
dotnet add package Fdw.Services.Catalog.Clients --prerelease
```

## Dependencies

`Fdw.Services.Catalog.Clients.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Endpoints` · `Fdw.Web.Http.Authentication`
