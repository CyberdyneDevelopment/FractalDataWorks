...
public class DuplicateTypeOptionAnalyzer : DiagnosticAnalyzer
{
    /// <summary>
    /// Diagnostic ID for duplicate enum option names.
    /// </summary>
    public const string DiagnosticId = "FDW026";
    ...
    private static string? GetOptionCollectionName(AttributeData attr, INamedTypeSymbol typeSymbol)
    {
        ...
        while (baseType != null)
        {
            var baseAttrs = baseType.GetAttributes()
                .Where(a => string.Equals(a.AttributeClass?.Name, "EnhancedEnumBaseAttribute", StringComparison.Ordinal) ||
                           string.Equals(a.AttributeClass?.Name, "EnumOptionBaseAttribute", StringComparison.Ordinal) ||
                           string.Equals(a.AttributeClass?.Name, "EnumOptionBase", StringComparison.Ordinal))
                .ToList();
