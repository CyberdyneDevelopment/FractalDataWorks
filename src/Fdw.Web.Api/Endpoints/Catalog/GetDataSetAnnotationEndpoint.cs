...
public abstract class GetDataSetAnnotationEndpoint : Endpoint<DataSetAnnotationRequest, DataSetAnnotationPayload>
{
    private readonly QualityConfigurationProvider _provider;

    /// <summary>Initializes a new instance of the <see cref="GetDataSetAnnotationEndpoint"/> class.</summary>
    /// <param name="provider">The configuration provider for quality and catalog data.</param>
    protected GetDataSetAnnotationEndpoint(QualityConfigurationProvider provider)
    {
        _provider = provider;
    }
