...
/// <summary>A dataverse as it appears in a list.</summary>
/// <remarks>
...
/// </remarks>
public class DataverseSummaryResponse : ResourceSummary
{
    // No MemberCount/ResourceCount: both were taken from the aggregate's child collections, which
    // the list path does not load, so they reported a confident 0 for a populated dataverse rather
    // than disagreeing with the map. No ModifiedAt: ModifyDate is NOT NULL with a default and is
    // stamped at insert, so a never-modified dataverse would report a modification, and no screen
    ...
    /// <summary>Gets or sets the one-line statement of what this dataverse is for.</summary>
    /// <remarks>
