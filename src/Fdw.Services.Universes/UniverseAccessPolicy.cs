...
/// <summary>
/// Decides universe write access from ownership and membership.
/// </summary>
/// <remarks>
/// <para>
/// Two ways to be allowed: own it, or hold a membership that is both in effect and carries a role
/// that may write. Both halves are read off their options — <see cref="UniverseMemberStateBase"/>
/// says whether someone is here, <see cref="UniverseMemberRoleBase"/> says what they may do — so a
...
/// keeps its universe access until the token is renewed — the same window every baked permission
/// has, not a property of this policy.
/// </para>
/// </remarks>
public sealed class UniverseAccessPolicy : IUniverseAccessPolicy
{
    private readonly IAuthenticationContextAccessor _authContext;
    private readonly RoleConfigurationProvider _roles;
    private readonly ILogger _logger;

    /// <summary>Initializes a new instance of the <see cref="UniverseAccessPolicy"/> class.</summary>
    ...
    public UniverseAccessPolicy(
        IAuthenticationContextAccessor authContext,
        RoleConfigurationProvider roles,
        ILogger<UniverseAccessPolicy>? logger = null)
    {
        _authContext = authContext ?? throw new ArgumentNullException(nameof(authContext));
        _roles = roles ?? throw new ArgumentNullException(nameof(roles));
        _logger = logger ?? NullLogger<UniverseAccessPolicy>.Instance;
    }

    /// <inheritdoc />
    public async Task<IGenericResult> MayWrite(
        UniverseConfiguration universe, CancellationToken cancellationToken = default)
    {
        if (universe is null) throw new ArgumentNullException(nameof(universe));

        if (_authContext.Current is not { } caller)
            return Deny(universe.Name, "no authentication context");

        if (!Guid.TryParse(caller.UserId, out var callerUserId))
            return Deny(universe.Name, "the caller's UserId is not a Guid");

        if (universe.OwnerUserId == callerUserId)
        ...
        if (callerRoleIds.IsFailure || callerRoleIds.Value is null)
            return Deny(universe.Name, "the caller's roles could not be read");

        return universe.Members.Any(m => Permits(m, callerUserId, callerRoleIds.Value))
            ? GenericResult.Success()
            : Deny(universe.Name, "the caller neither owns it nor holds a membership that may change it");
    }
    ...
    private static bool Permits(
        UniverseMemberConfiguration member, Guid callerUserId, IReadOnlyCollection<Guid> callerRoleIds)
    {
        if (!member.IsCurrent || member.IsDeleted)
            return false;

        if (UniverseMemberStates.ByName(member.State) is not { GrantsMembership: true })
            return false;

        if (UniverseMemberRoles.ByName(member.MemberRole) is not { MayWrite: true })
            return false;

        return UniverseSubjectTypes.ByName(member.SubjectType)
    ...
    private IGenericResult Deny(string universeName, string reason)
        => GenericResult.Failure(
            UniversesResultCodes.ByName("UniverseWriteNotPermitted"), _logger,
            ResultDetails.Create("name", universeName, "reason", reason));
}
