# Fdw.Services.SecretManagers.Clients

The API client for secret-manager configuration.
...
| Type | Kind | Purpose |
|---|---|---|
| `SecretManagerApiClient` | class | API client for secret manager endpoints. |

## Installation

```bash
dotnet add package Fdw.Services.SecretManagers.Clients --prerelease
```

## Dependencies

`Fdw.Services.SecretManagers.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Endpoints` · `Fdw.Web.Http.Authentication`
