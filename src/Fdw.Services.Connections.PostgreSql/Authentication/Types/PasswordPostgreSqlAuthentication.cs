...
public sealed class PasswordPostgreSqlAuthentication : PostgreSqlAuthenticationConfiguration
{
    ...
    /// <inheritdoc/>
    // Why: Password owns the SecretKeyName key — this method extracts it from `values`, calls
    // secretManager.Execute to resolve the password, then builds the fragment. The factory never
    // touches SecretKeyName/SecretManagerName/Username keys: the auth TypeOption holds them as its
    // declared Expected/Required properties.
