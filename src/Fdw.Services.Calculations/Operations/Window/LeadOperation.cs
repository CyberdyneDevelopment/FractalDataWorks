...
/// <summary>
/// Accesses a field value from a subsequent row within a partition.
/// Equivalent to SQL <c>LEAD(field, offset) OVER (PARTITION BY ... ORDER BY ...)</c>.
/// </summary>
[TypeOption(typeof(CalculationOperationTypes), "Lead")]
[ExcludeFromCodeCoverage]
public sealed class LeadOperation : CalculationOperationBase
{
    private readonly ILogger<LeadOperation> _logger;

    /// <summary>
    /// Initializes a new instance of the <see cref="LeadOperation"/> class.
    /// </summary>
    public LeadOperation()
        : base(id: 33, name: "Lead", category: "Window", description: "Accesses a field value from a subsequent row within a partition")
    {
        _logger = NullLogger<LeadOperation>.Instance;
        Parameters =
        [
            new OperationParameterDefinition { Name = "Field", Kind = "Field", IsRequired = true, DisplayName = "Value Field", HelpText = "The field to look ahead on" },
            new OperationParameterDefinition { Name = "Offset", Kind = "Scalar", IsRequired = true, DisplayName = "Offset", HelpText = "Number of rows to look ahead (default 1)" },
