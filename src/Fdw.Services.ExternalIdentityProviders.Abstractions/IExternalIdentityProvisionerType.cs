...
public interface IExternalIdentityProvisionerType : IServiceType
{
    ...
    /// <c>ExternalIdentityProvisionerTypes</c>'s own <c>AddScoped</c> factory via
    /// <see cref="Register"/>, once per domain provider instance it constructs — not once at
    /// startup — so whichever scope actually builds an instance (root at startup, a real request's
    /// own scope for a real request) is the same scope whose <c>IServiceProvider</c> the closure
    /// resolves against. Registering from each option's <c>Initialize</c> would run only once
    /// against the root container, so every scope but root's own would find this domain's factory
    /// registry empty.
    /// </remarks>
    /// <param name="method">
    ...
    /// construction used.
    /// </param>
    void Registration(Func<IServiceProvider, IExternalIdentityProvisionerServiceProvider?, IExternalIdentityProvisionerConfigurationProvider?, ILogger<IExternalIdentityProvisionerType>, IGenericResult> method);
    ...
    IGenericResult Register(IServiceProvider serviceProvider, IExternalIdentityProvisionerServiceProvider? domainProvider, IExternalIdentityProvisionerConfigurationProvider? domainConfigurationProvider, ILogger<IExternalIdentityProvisionerType> logger);
}
