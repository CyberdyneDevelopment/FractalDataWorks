# Fdw.Services.Notifications.Clients

The API client for notifications, rules and recipient lists.
...
| Type | Kind | Purpose |
|---|---|---|
| `NotificationApiClient` | class | API client for notification management — domain-level lists (notifications, rules, recipient lists) and… |

## Installation

```bash
dotnet add package Fdw.Services.Notifications.Clients --prerelease
```

## Dependencies

`Fdw.Services.Notifications.Abstractions` · `Fdw.Web.Clients` · `Fdw.Web.Http.Authentication`
