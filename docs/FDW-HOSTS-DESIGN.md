...
| `Fdw.Hosting.Ui` **(NEW)** | net10.0 | `FdwUiHost` / `FdwUiHostBuilder`. Refs **`Fdw.Hosting.Core` only** plus `Fdw.Web.Http.Authentication.Blazor`, `Fdw.UI.Registration`. **No ConfigurationGateway path exists in this package** — the UI-never-touches-ConfigurationDb seam is structural, not a runtime flag. |
